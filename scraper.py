		...
		for firstUrl in links:
			...
			for elm in urlSoup.find_all("p"):
				if urlPattern.search(elm.text):
					...
					global counter
					counter+=1
					print(str(counter) + "/" + str(len(links)) + " [FOUND] " + x)
...
def addSourceToServer():
	...
	for addedSource in sourceLinks:
		...
		request = requests.post("https://crithinkapp.com/api/admin/news-source-add", data=payload, headers={"Content-Type": "application/json", "X-CriThink-Cross-Service": "scraper"}, verify=False)
		if request.status_code == 204:
			counter+=1
			goodLog = str(counter) + "/" + str(len(sourceLinks)) + " [OK] " + addedSource
			print(goodLog)
			log.append(goodLog)
		else:
			badLog = str(counter) + "/" + str(len(sourceLinks)) + " [ERROR] " + addedSource + " [TYPE] " + str(request.json())
			print(badLog)
			log.append(badLog)
...
def makeLog():
	logName = "log/log_"+datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S")+"_"+classification+".txt"
